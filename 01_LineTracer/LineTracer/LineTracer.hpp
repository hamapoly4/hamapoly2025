...
private:
    /* インスタンス */
    RunSignal*      mStartSignal;       // 開始合図
    RunSignal*      mStopSignal;        // 停止合図
...
public:
    /* コンストラクタ */
    LineTracer(RunSignal* startsignal, RunSignal* stopsignal, Run* linetrace,
                DeviceMonitor* distancemonitor, DeviceMonitor* brightnessmonitor, DeviceMonitor* statemonitor)
        : mStartSignal(startsignal), mStopSignal(stopsignal), mLineTrace(linetrace),
